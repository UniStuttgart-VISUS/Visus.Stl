// <copyright file="SeasonalTrendLoessCompatTest.cs" company="Universität Stuttgart">
...
namespace Visus.Stl.Test {
    ...
    /// <summary>
    /// Compatibility test with Java implementation at
    /// https://github.com/ServiceNow/stl-decomp-4j/blob/master/stl-decomp-4j/src/test/java/com/github/servicenow/ds/stats/stl/SeasonalTrendLoessTest.java
    /// </summary>
    [TestClass]
    public sealed class SeasonalTrendLoessCompatTest {

        [TestMethod]
        public void PureSineTest() {
            long seed = 1234567L; // System.nanoTime() // change this to do random stress test

            double[] data = this._testDataGenerator.CreateNoisySeasonalData(
                144, 12, 1.0, 0.0, 0.0, seed);

            var builder = new SeasonalTrendLoessBuilder()
                .SetPeriodLength(12)
                .SetSeasonalWidth(7)
                .SetNonRobust();
            var smoother = builder.Build(data);

            var stl = smoother.Decompose();

            for (int i = 0; i < data.Length; ++i) {
                Assert.AreEqual(data[i], stl.Seasonal[i], 1.0e-14);
                Assert.AreEqual(0.0, stl.Trend[i], 1.0e-14);
                Assert.AreEqual(0.0, stl.Residuals[i], 1.0e-14);
            }
        }
        ...
        public void PureTrendTest() {
            ...
            for (int i = 0; i < data.Length; ++i) {
                Assert.AreEqual(data[i], stl.Trend[i], 1.0e-12);
                Assert.AreEqual(0.0, stl.Seasonal[i], 1.0e-12);
                Assert.AreEqual(0.0, stl.Residuals[i], 1.0e-12);
                Assert.AreEqual(1.0, stl.Weights[i], 1.0e-13);
            }
        }

        [TestMethod]
        public void SquareWaveTest() {
            double[] data = this._testDataGenerator.CreateSquareWaveData();
            ...
            for (int i = 0; i < data.Length; ++i) {
                Assert.AreEqual(42.5, stl.Trend[i], 1.0e-12);
                Assert.AreEqual(data[i], stl.Seasonal[i] + stl.Trend[i], 1.0e-12);
                Assert.AreEqual(0.0, stl.Residuals[i], 1.0e-12);
                Assert.AreEqual(1.0, stl.Weights[i], 1.0e-13);
            }
        }
        ...
        public void ForcedPeriodicityTest() {
            // To force periodicity, smooth the cyclic sub-series with a zero
            // degree polynomial and a very long window, causing it to
            // interpolate each sub-series point at the average for the
            // subseries.
            ...
            var builder = new SeasonalTrendLoessBuilder()
                .SetPeriodLength(12)
                .SetSeasonalWidth(100000001).SetSeasonalDegree(0).SetSeasonalJump(100000001)
                .SetTrendWidth(23)
                .SetLowpassWidth(13)
                .SetInnerIterations(2).SetRobustnessIterations(0);
            var smoother = builder.Build(data);
            ...
            for (int i = 0; i < 12; ++i) {
                for (int p = 0; p < 12; ++p) {
                    Assert.AreEqual(stl.Seasonal[i], stl.Seasonal[i + p * 12], epsilon);
                }
            }
        }
        ...
        // Test data obtained from Python

        private readonly double[,] fNonRobustNoisySinusoidResults = new double[,] {
        ...
        private static readonly double[,] fRobustNoisySinusoidResults = new double[,] {
        ...
        private readonly StlTestDataGenerator _testDataGenerator
            = new StlTestDataGenerator();
        #endregion
    }
}
